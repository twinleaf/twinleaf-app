...
- Hovering a graph reads the sample nearest the pointer into the legend. Holding Option frees the cursor from the data, so it reads the axes at the pointer itself.
- Each capture RPC (`*.capture`) that is opened plots its record in a pane of its own below the graphs, with the graphs' cursor and legend and its own Auto re-trigger. A pane can move to its own window and takes its Auto and axis choices along; closing that window stacks it again. The pane's FFT toggle shows the record's power spectral density instead, in units²/Hz on log or linear axes: a Hann-windowed periodogram of the whole record, scaled like the live FFT so its square root is that display's ASD.
